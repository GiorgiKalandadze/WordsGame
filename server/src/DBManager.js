...
class DBManager {
	...
	static async listAllCollections(databaseName) {
		try {
			const collectionsList = await DBManager.client
				.db(databaseName)
				.listCollections()
				.toArray();
	...
	static async insertDocument(dbName, collectionName, newDocument) {
		try {
			...
			throw new Error(
				`Error inserting document in collection '${collectionName}': ${error}`
			);
		}
	}
	...
	static async insertManyDocuments(dbName, collectionName, newDocumentsList) {
		try {
			const result = await DBManager.client
				.db(dbName)
				.collection(collectionName)
				.insertMany(newDocumentsList);
			console.log('Insert many documents result: ', result);
		} catch (error) {
			throw new Error(
				`Error inserting many document in collection '${collectionName}': ${error}`
			);
		}
	}

	static async getManyDocuments(
		dbName,
		collectionName,
		filter = {},
		skip = 0,
		...
		try {
			const cursor = await DBManager.client
				.db(dbName)
				.collection(collectionName)
				.find(filter)
				.skip(skip)
			...
			throw new Error(
				`Error getting documents from collection '${collectionName}': ${error}`
			);
		}
	}
	...
	static async getDocument(dbName, collectionName, filter) {
		try {
			return await DBManager.client
				.db(dbName)
				.collection(collectionName)
				.findOne(filter);
		} catch (error) {
			throw new Error(
				`Error getting document from collection '${collectionName}': ${error}`
			);
		}
	}
	...
	static async updateDocument(dbName, collectionName, filter, newDocument) {
		try {
			...
			throw new Error(
				`Error getting updating document in collection '${collectionName}': ${error}`
			);
		}
	}
	...
	static async removeDocument(dbName, collectionName, filter) {
		try {
			...
			throw new Error(
				`Error removing document from collection '${collectionName}': ${error}`
			);
		}
	}

	static async getCountOfDocuments(dbName, collectionName) {
		try {
			return await DBManager.client
				.db(dbName)
				.collection(collectionName)
			...
			throw new Error(
				`Error getting count of documents in collection '${collectionName}': ${error}`
			);
		}
	}
}
